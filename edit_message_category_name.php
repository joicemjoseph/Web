...
<tr><td colspan="3" class="style7" align="left">Edit SMS Category>></td></tr>
<tr><td colspan="3" align="center">&nbsp;</td></tr>
	  ...
	  $username=$_GET['username']; //echo $username;
	   ...
	   $cnt=mysql_query("select * from sms_category WHERE id=$id");

      $row=mysql_fetch_array($cnt); 
	  ?>
	  
	   <form method="post" action="edit_message_category_name.php" name="msgform" id="msgform">      
 <table border="0" width="100%" align="center"  class="style7">
 <tr><td width="100">&nbsp;</td><td align="center" bgcolor="#f4f4f4"><br />
 <font color="red"> Note :</font> The symbol <font color="red">'</font>
  (Single Quote) is not allowed.<br /><br />
   <label>
    <textarea name="update_category" id="update_category" cols="40" rows="2"> <?php echo $row[0];?></textarea>
     </label>
	 <label>
		  ...
		  <label>
          <input type="submit" name="Submit" id="Submit" value="Update Category" onClick="return validate();">
          </label> <br /><br />
...
<?php
echo "<a href='edit_message_category_name.php?id=$row[1]&value=edit&username=$username'>
<img src=images/edit.png border='0'></img></a>";
?>
...
if(isset($_POST['Submit']))
{
 ...
 $username=$_POST['username']; //echo $username;
  ...
  $result=mysql_query("update sms_category set category='$update_category' WHERE id='$id'");
  $result_category_inbox=mysql_query("update messages_inbox set category='$update_category' WHERE category_id='$id'");
  
   if($result==1)
 {
   $username=$_POST['username'];
 echo "<script>alert('Category Name Updated Successfully ...');</script>";
 echo "<script>document.location.href='edit_message_category_name.php?username=$username'</script>";
 }
  else
 {
 $username=$_POST['username'];
